...
It is important to note that $\mathbf{B}\mathbf{B}^T$ is singular in cases where bins contain overlapping samples, so the complex multivariate Gaussian distribution must be generalized to the singular covariance case.
...
\red{FIX
\begin{align*}
    p(\mathbf{b} | \mathbf{x}) = &f_{\mathbf{B}\eta+\zeta} \left( \mathbf{Bb} - \mathbf{A}\mathbf{x} \right) \\
    = & \frac{\exp\left[ - (\mathbf{Bb} - \mathbf{A}\mathbf{x})^H \Sigma_{\mathbf{B}\eta+\zeta} ^{\dagger}(\mathbf{Bb} - \mathbf{A}\mathbf{x})\right]}{(\pi)^{\tilde{M}QN_\mathrm{frames}} \det{ |\Sigma_{\mathbf{B}\eta+\zeta}|}}
\end{align*}
}
...
\begin{equation} \label{eq:map_gls_sol}
    \mathbf{x}_\mathrm{MAP} = \underset{\mathbf{x}}{\operatorname{argmin}}\
            \left\|(\Sigma_{\mathbf{B}\eta + \zeta}^\dagger)^{1/2}(\mathbf{Bb} - \mathbf{A}\mathbf{x})\right\|_2^2
...
The most general and data-accurate approach to the problem would consider any arbitrary noise covariance matrix, $\Sigma_{\mathbf{B}\eta + \zeta}$ under the constraints:
\begin{align}
    \Sigma_{\mathbf{B}\eta} \propto &\mathbf{B}\mathbf{B}^T \label{constraint:cov_beta} \\
    (\Sigma_\zeta)_{m,m} = &0, \quad \forall m \in \{m: t_m \in \{\tau_f\}_f^{N_\mathrm{frames}}\} \label{constraint:cov_zeta}
\end{align}
The first constraint in Eq. \ref{constraint:cov_beta} enforces homoscedasticity in the thermal noise component and to account for the sample overlap.
The second constraint in Eq. \ref{constraint:cov_zeta} ensures that the model is tuned to weight each solution around the nominal time points $\tau_f$, by enforcing $\zeta = 0$ at these points.

If the covariance is unknown, we can jointly estimate the covariance and image timeseries using the following constrained optimization problem:
\begin{equation*}
    \begin{aligned}
        \mathbf{x}_* = &\underset{\mathbf{x}}{\operatorname{argmin}} &\min_{\Sigma_{\mathbf{B}\eta + \zeta}}
        \left\|\Sigma_{\mathbf{B}\eta + \zeta}^{-1/2}(\mathbf{Bb} - \mathbf{A}\mathbf{x})\right\|_2^2
            + \frac{1}{\sigma_{\mathbf{x}}^2}\left\|\mathbf{x}\right\|_2^2 \\
            & \text{\bf{s.t.}} & \Sigma_{\mathbf{B}\eta + \zeta} = \Sigma_{\mathbf{B}\eta} + \Sigma_{\zeta} \\
            &&  \Sigma_{\mathbf{B}\eta} \propto \mathbf{B}\mathbf{B}^T \\
            && (\Sigma_\zeta)_{m,m} = 0, \quad \forall m \in \{m: t_m \in \{\tau_f\}_f^{N_\mathrm{frames}}\} 
    \end{aligned}
\end{equation*}

\red{computing the exact covariance matrix would be the most statistically accurate approach.
however, 1. it is an extremely ill-posed problem, and 2. we might want to allow some degree of model mismatch anyways in order to increase the contribution of far-away k-space samples}
