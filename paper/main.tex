...
Each frame $f$ is indexed by a subset of $\tilde{M}$ sample indices $\mathcal{M}_f \subset \{1, ..., M\}$, and the corresponding binning matrix $\mathbf{B}_f \in \{0,1\}^{\tilde{M} \times M}$ extracts these samples from a vector of length $M$.
...
\begin{equation} \label{eq:binned_data}
    \begin{aligned}
        ...
        \begin{bmatrix}
            \mathbf{B}_1 \\ \mathbf{B}_2 \\ \vdots \\ \mathbf{B}_{N_\mathrm{frames}}
        \end{bmatrix}
        \begin{bmatrix}
...
\begin{equation*}
    \mathbf{A} = \begin{bmatrix}
        ...
        \vdots & \vdots & \ddots & \vdots \\
        \mathbf{0} & \mathbf{0} & ... & \mathbf{A}_{N_\mathrm{frames}}
    \end{bmatrix}
\end{equation*}
where $\mathbf{A}_f \in \mathbb{C}^{\tilde{M}Q \times N}$ is the acquisition operator for frame $f$ reflecting a discretized form of Eq. \ref{eq:signal_eq} on the $N$-voxel discrete grid corresponding to $\mathbf{x}$, including coil sensitivity encoding and Fourier encoding at k-space coordinates $\{\vec{\omega}(\mathbf{t}_m)\}_{m \in \mathcal{M}_f}$.
...
We can derive that $\mathbf{B}\eta \sim \mathcal{CN}(\mathbf{0}, \sigma_\epsilon^2\mathbf{B}\mathbf{B}^T)$ (see proof in Appendix \ref{proofs}).
...
\subsubsection{LSCAP Approach}
The most general case could simultaneously solve for the image and noise covariance using the Least Squares Covariance Adjustment Problem (LSCAP) \cite{boyd_2002}:

However, the LSCAP method is computationally expensive and slow to converge.
...
\subsection{Virtual Phantom Experiments}
\subsection{Looping Star fMRI Experiments}

\section{Appendix}
\subsection{Proofs} \label{proofs}
\begin{proof}[Derivation of $\mathbf{B}\eta$ covariance matrix (Sec. \ref{sec:binned_acquisition_model})]
    \begin{align*}
