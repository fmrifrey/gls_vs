...
\begin{align*}
    ...
    = &\mathbb{P}(\mathbf{B} \eta + \zeta = \mathbf{B} \mathbf{b} - \mathbf{A}\mathbf{x})
\end{align*}

Since $\mathbf{B} \eta$ and $\zeta$ are both multivariate complex Gaussian, $\mathbf{B} \eta+\zeta$ is also multivariate complex Gaussian \red{(is this true if $\mathbf{B}\eta$ is singular??)} with covariance matrix:
\begin{equation} \label{eq:noise_covariance}
    \begin{aligned}
        \Sigma_{\mathbf{B} \eta+ \zeta} = &\sigma_{\epsilon}^2 \mathbf{B} \mathbf{B}^T + \Sigma_\zeta  \\
        = & \operatorname{Cov}(\mathbf{B} \mathbf{b} - \mathbf{A}\mathbf{x})
    \end{aligned}
\end{equation}
Then, under the assumption that $\Sigma_{\mathbf{B} \eta+ \zeta}$ is non-singular:
\begin{align*}
    p(\mathbf{b} | \mathbf{x}) = &f_{\mathbf{B} \eta+ \zeta} \left( \mathbf{B} \mathbf{b} - \mathbf{A}\mathbf{x} \right) \\
    = & \frac{\exp\left[ - (\mathbf{B} \mathbf{b} - \mathbf{A}\mathbf{x})^H \Sigma_{\mathbf{B} \eta+ \zeta} ^{-1}(\mathbf{B} \mathbf{b} - \mathbf{A}\mathbf{x})\right]}{(\pi)^{\tilde{M}QN_\mathrm{frames}} \det{ |\Sigma_{\mathbf{B} \eta+ \zeta}|}}
\end{align*}
...
\begin{equation} \label{eq:map_gls_sol}
    \mathbf{x}_\mathrm{MAP} = \underset{\mathbf{x}}{\operatorname{argmin}}\
        \left\|\Sigma_{\mathbf{B} \eta + \zeta}^{-1/2}(\mathbf{B} \mathbf{b} - \mathbf{A}\mathbf{x})\right\|_2^2
        + \frac{1}{\sigma_{\mathbf{x}}^2}\left\|\mathbf{x}\right\|_2^2
\end{equation}
This solution is robust to any arbitrary binning strategy and is statistically optimal given a good estimate for $\Sigma_{\mathbf{B}\eta + \zeta}$ (we will discuss approaches for estimating the covariance in Section \ref{sec:estimating_noise_covariance}).
...
\red{(not sure if this is actually possible)}

\subsubsection{Inverse Approximations}
\red{(i.e. FBP, DCF-NUFFT with the correct weighting for fast recon applications such as real-time)}

\subsection{Uncertainty Quantification}
\red{(idea: by not using complicated regularizers to dealias, UQ should be easier)}
